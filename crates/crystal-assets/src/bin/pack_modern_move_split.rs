...
fn main() -> Result<()> {
    ...
    let repository_root = PathBuf::from(
        args.next()
            .context("usage: pack_modern_move_split <repository-root> [base-pack] [output-pack]")?,
    )
    .canonicalize()
    ...
    let output_pack = args
        .next()
        .map(PathBuf::from)
        .unwrap_or_else(|| repository_root.join("content-packs/modern-move-split.crystalpack"));
    if args.next().is_some() {
        anyhow::bail!("usage: pack_modern_move_split <repository-root> [base-pack] [output-pack]");
    }
    pack_modern_move_split(&base_pack, &output_pack)
}

fn pack_modern_move_split(base_pack: &Path, output_pack: &Path) -> Result<()> {
    ...
    let modern_move_split = build_modern_move_split_modpack(&base)?;
    ...
    modern_move_split
        ...
        .with_context(|| format!("write modern move split pack {}", output_pack.display()))?;
