...
use crystal_assets::{build_player_customization_modpack, read_verified_compiled_game_pack};
...
fn pack_player_customization(base_pack: &Path, output_pack: &Path) -> Result<()> {
    ...
    let player_customization = build_player_customization_modpack(&base)?;
    ...
    player_customization
        ...
        .with_context(|| format!("write player customization pack {}", output_pack.display()))?;
