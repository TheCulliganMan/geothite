...
#[test]
fn scripted_trainer_type_follows_loadvar_before_startbattle() {
    let scripts = BTreeMap::from([("RivalScene".to_string(), serde_json::json!([
        {"command":"loadtrainer","args":["RIVAL1","RIVAL1_1_TOTODILE"]},
        {"command":"loadvar","args":["VAR_BATTLETYPE","BATTLETYPE_CANLOSE"]},
        {"command":"startbattle","args":[]},
        {"command":"loadvar","args":["VAR_BATTLETYPE","BATTLETYPE_TRAINER"]},
        {"command":"loadtrainer","args":["RIVAL1","RIVAL1_2_TOTODILE"]},
        {"command":"startbattle","args":[]}
    ]))]);
    let battles = parse_scripted_trainer_battles("CherrygroveCity", &scripts).unwrap();
    assert_eq!(battles[0].request.battle_type, "BATTLETYPE_CANLOSE");
    assert_eq!(battles[1].request.battle_type, "BATTLETYPE_TRAINER");
}
