...
fn stage_visible_battle_messages(
    ...
    for event in events {
            ...
            if stage_message_scenes {
                for _ in event_message_count_before..runtime_shell.battle_messages.len() {
                    runtime_shell
                        ...
                        .push_back(Arc::new(event_scene.clone()));
                }
            }
            continue;
        }
                ...
                if event_message_count_before == runtime_shell.battle_messages.len() {
                    ...
                    if let Some(trigger_message) = runtime_shell.battle_messages.back().cloned() {
                        runtime_shell
                            ...
                            .push_back((trigger_message, Arc::new(event_scene.clone())));
                    }
                }
            }
            continue;
        ...
        if stage_message_scenes {
            for _ in event_message_count_before..runtime_shell.battle_messages.len() {
                runtime_shell.battle_message_scenes.push_back(Arc::new(
                    if matches!(
                        event,
                        BattleEvent::Fainted { .. }
            ...
            {
                *scene = Arc::new(event_scene_before.clone());
            }
            if let BattleEvent::Fainted { side } = event {
                ...
                runtime_shell
                    .pending_battle_scenes_after_message
                    .push_back((trigger, Arc::new(event_scene.clone())));
            }
                    ...
                    deferred_airborne_end_scenes.remove(&(*side, move_name.as_str()))
            {
                ...
                runtime_shell
                    .pending_battle_scenes_after_message
                    .push_back((trigger, Arc::new(revealed_scene.clone())));
                ...
                && battle_event_changes_visible_scene(event)
            {
                ...
                if let Some(scene) = runtime_shell.battle_message_scenes.back_mut() {
                    *scene = Arc::new(event_scene.clone());
                }
            }
...
fn resolve_visible_wild_faint_prompt(
    ...
    runtime_shell.battle_message_scene = Some(Arc::new(snapshot));
...
fn switch_visible_battle_party_without_turn(
    ...
    runtime_shell.battle_message_scene = Some(Arc::new(replacement));
    mark_runtime_snapshot_dirty(runtime_shell);
...
fn switch_visible_trainer_shift_party_without_turn(
    ...
    runtime_shell
        .battle_message_scenes
        .push_back(Arc::new(replacement.clone()));
...
fn complete_visible_standard_capture(
    ...
    if let (Some(previous), Some(candidate)) = (
        ...
        runtime_shell.battle_message_scene = Some(Arc::new(battle_before_completion));
    ...
    if let Some(stored) = completion.stored.as_ref() {
        if matches!(
            ...
            runtime_shell.battle_message_scene = Some(Arc::new(battle_before_completion.clone()));
        }
    }
    runtime_shell.last_audio_events.push(format!(
...
fn retain_visible_pre_reward_battle_scene(
    ...
    if runtime_shell.battle_message_scene.is_none() {
        runtime_shell.battle_message_scene = Some(Arc::new(battle_before_rewards.clone()));
    }
    ...
    while runtime_shell.battle_message_scenes.len() < runtime_shell.battle_messages.len() {
        runtime_shell.battle_message_scenes.push_back(Arc::new(battle_before_rewards.clone()));
    }
    mark_runtime_snapshot_dirty(runtime_shell);
}
