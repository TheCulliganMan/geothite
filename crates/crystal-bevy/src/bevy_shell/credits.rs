...
fn selected_party_index(runtime_shell: &mut BevyRuntimeShell) -> Result<usize> {
    let snapshot = runtime_shell.shell.snapshot()?;
    if runtime_shell.bill_pc_deposit_open {
        let VisiblePcPokemonLocation::Party(index) = selected_visible_pc_pokemon_location(runtime_shell, &snapshot)? else {
            anyhow::bail!("PC Deposit must select a party Pokemon");
        };
        return Ok(index);
    }
    if snapshot.party.slots.is_empty() {
...
fn refresh_visible_battle_scene_after_party_progression(
    ...
    if latest.battle.is_some() {
        runtime_shell.battle_message_scene = Some(Arc::new(latest));
        mark_runtime_snapshot_dirty(runtime_shell);
    }
    Ok(())
}
...
fn move_visible_storage_cursor(runtime_shell: &mut BevyRuntimeShell, delta: isize) -> Result<()> {
    let snapshot = runtime_shell.shell.snapshot()?;
    let party = runtime_shell.bill_pc_deposit_open
        || (runtime_shell.bill_pc_move_open && runtime_shell.bill_pc_move_party_open);
    // CopyBoxmonSpecies appends the -1/CANCEL entry in every BillsPC mode,
    // including the Move insertion list.
    let (surface_id, count) = if party {
        (pc_party_surface_id().to_string(), snapshot.party.slots.len() + 1)
    } else {
        (storage_cursor_surface_id(visible_pc_box_index(&snapshot, runtime_shell)),
            visible_storage_box(&snapshot, runtime_shell)?.slots.len() + 1)
    };
    let current = strict_readonly_cursor_index(&runtime_shell.storage_cursor, &surface_id, count)
    ...
    // BillsPC_PressUp/Down scroll only after the cursor reaches a window edge.
    if next < runtime_shell.pc_list_scroll {
        runtime_shell.pc_list_scroll = next;
    } else if next >= runtime_shell.pc_list_scroll + 5 {
        runtime_shell.pc_list_scroll = next - 4;
    }
    runtime_shell.storage_cursor = Some(MenuCursor { surface_id, option_index: next });
    runtime_shell.last_audio_events.push(format!("cursor {}->{}", current + 1, next + 1));
    trim_event_log(&mut runtime_shell.last_audio_events);
    Ok(())
}
...
fn move_visible_pc_item_cursor(runtime_shell: &mut BevyRuntimeShell, delta: isize) -> Result<()> {
    ...
    runtime_shell.ball_cursor = None;
    runtime_shell.tmhm_cursor = None;
    let count = snapshot.bag.pc_items.len() + 1;
    let current = strict_readonly_cursor_index(&runtime_shell.pc_item_cursor, "pc:items", count)
        .context("PC item list requires an item or CANCEL cursor")?;
    let next = current.saturating_add_signed(delta).min(count - 1);
    if next == current {
        return Ok(());
    }
    if next < runtime_shell.pc_item_scroll {
        runtime_shell.pc_item_scroll = next;
    } else if next >= runtime_shell.pc_item_scroll + 4 {
        runtime_shell.pc_item_scroll = next - 3;
    }
    runtime_shell.pc_item_row = next - runtime_shell.pc_item_scroll;
    ...
    runtime_shell
        .last_audio_events
        .push(format!("cursor {}->{}", current + 1, next + 1));
    trim_event_log(&mut runtime_shell.last_audio_events);
...
fn selected_current_box_slot_index(runtime_shell: &mut BevyRuntimeShell) -> Result<usize> {
    ...
    let surface_id = storage_cursor_surface_id(current_box.index);
...
fn visible_pc_box_index(snapshot: &RuntimeShellSnapshot, runtime_shell: &BevyRuntimeShell) -> usize {
    if runtime_shell.bill_pc_move_open { runtime_shell.bill_pc_move_loaded_box }
    else { snapshot.storage.current_pc_box }
}

fn visible_storage_box<'a>(snapshot: &'a RuntimeShellSnapshot, runtime_shell: &BevyRuntimeShell) -> Result<&'a crate::RuntimePcBoxSnapshot> {
    let index = visible_pc_box_index(snapshot, runtime_shell);
    snapshot.storage.boxes.iter().find(|pc_box| pc_box.index == index)
        .with_context(|| format!("visible PC box {index} is missing from storage snapshot"))
}
...
fn pc_party_surface_id() -> &'static str {
    "pc:party"
}
...
fn move_visible_nonwrapping_cursor_slot(
    cursor_slot: &mut Option<MenuCursor>,
    surface_id: String,
    option_count: usize,
    delta: isize,
    ...
    let current = strict_readonly_cursor_index(cursor_slot, &surface_id, option_count)
        .with_context(|| format!("menu {surface_id} has no valid cursor"))?;
    let next = current.saturating_add_signed(delta).min(option_count - 1);
    if next == current { return Ok(()); }
    *cursor_slot = Some(MenuCursor { surface_id, option_index: next });
    event_log.push(format!("cursor {}->{}", current + 1, next + 1));
    trim_event_log(event_log);
    Ok(())
}

/// ScrollingMenu_InitFlags never enables wrapping. Each pocket keeps its
/// scroll byte when its cursor moves back from the bottom of the window.
fn move_visible_pack_cursor_slot(
    cursor_slot: &mut Option<MenuCursor>,
    surface_id: String,
    option_count: usize,
    delta: isize,
    ...
    const ROWS: usize = 5;
    anyhow::ensure!(option_count > 0, "{surface_id} has no CANCEL row");
    let mut current = match cursor_slot.as_ref() {
        Some(cursor) => {
            anyhow::ensure!(cursor.surface_id == surface_id, "Pack {surface_id} has cursor surface {}", cursor.surface_id);
            cursor.option_index
        }
        None => 0,
    };
    if delta == 0 {
        // InitScrollingMenuCursor clamps the scroll first, preserving the
        // screen row after an item disappears. Only an invalid row then resets.
        let mut row = current
            .checked_sub(*scroll)
            .context("Pack cursor precedes its retained scrolling window")?;
        *scroll = (*scroll).min(option_count.saturating_sub(ROWS));
        if *scroll + row >= option_count {
            *scroll = 0;
            row = 0;
        }
        if row >= ROWS {
    ...
    anyhow::ensure!(
        current < option_count && current >= *scroll && current < *scroll + ROWS,
        "Pack cursor is outside its source window"
    );
    let next = current.saturating_add_signed(delta).min(option_count - 1);
    if next < *scroll {
        *scroll = next;
    } else if next >= *scroll + ROWS {
        *scroll = next + 1 - ROWS;
    }
    *cursor_slot = Some(MenuCursor {
        surface_id,
...
fn visible_text_label_can_auto_continue(runtime_shell: &BevyRuntimeShell) -> Result<bool> {
    if visible_pending_text_wait_closes_window(runtime_shell) {
        return Ok(false);
    }
    let snapshot = runtime_shell.shell.snapshot()?;
    Ok(snapshot.ui.text_window_open
        ...
        && !visible_field_text_ends_with_prompt(&snapshot, runtime_shell)?)
}
...
fn selected_visible_pc_pokemon_location(runtime_shell: &BevyRuntimeShell, snapshot: &RuntimeShellSnapshot) -> Result<VisiblePcPokemonLocation> {
    if runtime_shell.bill_pc_deposit_open || (runtime_shell.bill_pc_move_open && runtime_shell.bill_pc_move_party_open) {
        let selected = strict_readonly_cursor_index(&runtime_shell.storage_cursor,
            pc_party_surface_id(), snapshot.party.slots.len()).context("PC party selection is invalid")?;
        Ok(VisiblePcPokemonLocation::Party(snapshot.party.slots[selected].index))
    } else {
        let pc_box = visible_storage_box(snapshot, runtime_shell)?;
        let selected = strict_readonly_cursor_index(&runtime_shell.storage_cursor,
            &storage_cursor_surface_id(pc_box.index), pc_box.slots.len()).context("PC box selection is invalid")?;
        Ok(VisiblePcPokemonLocation::Box { box_index: pc_box.index, box_slot: pc_box.slots[selected].index })
    }
}

fn visible_pc_summary_pokemon(snapshot: &RuntimeShellSnapshot, location: VisiblePcPokemonLocation) -> Result<crate::core::models::pokemon::Pokemon> {
    let mut pokemon = visible_pc_pokemon_at(snapshot, location)?.clone();
    if matches!(location, VisiblePcPokemonLocation::Box { .. }) {
        // BillsPC_CopyMon calls CalcBufferMonStats for boxed records. The
        // temporary party-only bytes are rebuilt without changing the box.
        let stats = crate::core::models::pokemon::calculate_stats(&pokemon.species, pokemon.level, pokemon.dvs,
            crate::core::models::pokemon::StatExperience {
                hp: pokemon.hp_exp, attack: pokemon.attack_exp, defense: pokemon.defense_exp,
                speed: pokemon.speed_exp, special: pokemon.special_exp,
            });
        ...
        pokemon.hp = if pokemon.is_egg { 0 } else { stats.max_hp };
        pokemon.status = None;
...
fn move_visible_pc_summary_pokemon(runtime_shell: &mut BevyRuntimeShell, delta: isize) -> Result<()> {
    if !visible_wait_sfx_finished(runtime_shell) {
        return Ok(());
    }
    let snapshot = runtime_shell.shell.snapshot()?;
    let summary = runtime_shell.bill_pc_pokemon_summary.as_ref()
        .context("PC Stats browsing requires an open summary")?;
    let (surface, count) = match summary.location {
        VisiblePcPokemonLocation::Party(_) => (pc_party_surface_id().to_string(), snapshot.party.slots.len()),
        VisiblePcPokemonLocation::Box { box_index, .. } => {
            let pc_box = visible_storage_box(&snapshot, runtime_shell)?;
            anyhow::ensure!(pc_box.index == box_index, "PC Stats box differs from the visible list");
            (storage_cursor_surface_id(box_index), pc_box.slots.len())
        }
    };
    // _StatsScreenDPad excludes the -1/CANCEL entry before calling the
    // same five-row BillsPC_PressUp/Down routines as the underlying list.
    let current = strict_readonly_cursor_index(&runtime_shell.storage_cursor, &surface, count)
        .context("PC Stats requires a selected Pokemon")?;
    let next = current.saturating_add_signed(delta).min(count - 1);
    if next == current { return Ok(()); }
    move_visible_storage_cursor(runtime_shell, next as isize - current as isize)?;
    let location = selected_visible_pc_pokemon_location(runtime_shell, &snapshot)?;
    runtime_shell.bill_pc_pokemon_summary.as_mut()
        .context("PC Stats disappeared while browsing")?.location = location;
    queue_visible_stats_egg_sound(runtime_shell, visible_pc_pokemon_at(&snapshot, location)?)?;
    // MonStatsInit redraws the selected Pokemon without resetting page bits.
    mark_runtime_snapshot_dirty(runtime_shell);
    Ok(())
}

fn visible_pc_pokemon_at(snapshot: &RuntimeShellSnapshot, location: VisiblePcPokemonLocation) -> Result<&crate::core::models::pokemon::Pokemon> {
    match location {
        VisiblePcPokemonLocation::Party(index) => snapshot.party.slots.iter()
            .find(|slot| slot.index == index).map(|slot| &slot.pokemon)
            .with_context(|| format!("PC party slot {index} is missing")),
        VisiblePcPokemonLocation::Box { box_index, box_slot } => snapshot.storage.boxes.iter()
            .find(|pc_box| pc_box.index == box_index).and_then(|pc_box| pc_box.slots.iter().find(|slot| slot.index == box_slot))
            .map(|slot| &slot.pokemon).with_context(|| format!("PC box {box_index} slot {box_slot} is missing")),
    }
}

fn close_visible_pc_item_list(runtime_shell: &mut BevyRuntimeShell) {
    let action_index = match runtime_shell.pc_item_action {
        Some(VisiblePlayerPcAction::TossItem) => 2,
        _ => 0,
    };
    if let Some(cursor) = runtime_shell.pc_item_cursor.as_ref() {
        runtime_shell.pc_item_row = cursor.option_index - runtime_shell.pc_item_scroll;
    }
...
fn restore_visible_pc_item_list_position(runtime_shell: &mut BevyRuntimeShell) -> Result<()> {
    let total = runtime_shell.shell.snapshot()?.bag.pc_items.len() + 1;
    let mut row = if let Some(cursor) = runtime_shell.pc_item_cursor.as_ref() {
        cursor
            .option_index
            .checked_sub(runtime_shell.pc_item_scroll)
            .context("PC item cursor precedes its scrolling window")?
    } else {
        runtime_shell.pc_item_row
    };
    anyhow::ensure!(row < 4, "PC item row exceeds the source window");
    // InitScrollingMenuCursor adjusts the scroll first, preserving the
    // screen row. Only an out-of-list row resets the whole selection.
    runtime_shell.pc_item_scroll = runtime_shell.pc_item_scroll.min(total.saturating_sub(4));
    if runtime_shell.pc_item_scroll + row >= total {
        runtime_shell.pc_item_scroll = 0;
        row = 0;
    }
    runtime_shell.pc_item_row = row;
    runtime_shell.pc_item_cursor = Some(MenuCursor {
        surface_id: "pc:items".to_string(),
        option_index: runtime_shell.pc_item_scroll + row,
    });
    // The restored ScrollingMenu must reach its first WaitBGMap before input.
    runtime_shell.pc_menu_input_wait_frames = 4;
    mark_runtime_presentation_dirty(runtime_shell);
    Ok(())
}
