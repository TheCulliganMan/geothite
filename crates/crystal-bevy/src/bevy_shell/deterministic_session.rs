...
fn press_visible_b_button(runtime_shell: &mut BevyRuntimeShell) -> Result<()> {
    // Item receipts use A/B as the same text prompt, including the fanfare
    // fence and the following pocket page. The generic B notice dismissal
    // dropped the visible text but left the item state machine waiting,
    // resurrecting the aide's old offer and stranding its script cursor.
    if runtime_shell.visible_field_item_notice.is_some() && runtime_shell.field_notice.is_some() {
