...
fn selected_field_pack_item_id_from_snapshot(
    ...
    match pocket {
        FieldPackPocket::Items if snapshot.battle.is_some() => {
            let ids=carried_battle_non_ball_item_ids(snapshot);
            strict_readonly_cursor_index(&runtime_shell.bag_cursor,"battle:bag-items",field_pack_selectable_count(ids.len()))
