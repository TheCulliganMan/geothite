...
fn queue_existing_entity_despawn(
    ...
    // Browser 2D sprites are children of persistent fullscreen roots. Plain
    // despawn leaves dead IDs in Children; each new glyph then scans a list
    // that grows for the whole battle. Remove the hierarchy link as well.
...
fn render_playfield(
    ...
    if snapshot.battle.is_none() && !runtime_shell.follower_visible_tile_overrides.is_empty() {
        ...
        && let Some(battle) = &snapshot.battle
    {
        ...
        // Party screens replace the HUD; its sprites otherwise bleed through
        // the party background into the names and HP rows.
        if runtime_shell.battle_switch_cursor.is_none()
            && runtime_shell.battle_pack_target_mode.is_none()
