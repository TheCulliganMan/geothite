fn battle_party_action_entries(selected: usize) -> Vec<String> {
    ["SWITCH", "STATS", "CANCEL"].iter().enumerate()
        .map(|(i, label)| format!("{}{label}", if i == selected { ">" } else { " " }))
        .collect()
}

// Reserve space for the decision-critical suffix before shortening the name.
fn compatibility_party_label(party: &str, able: bool) -> String {
    let suffix = if able { " ABLE" } else { " NOT ABLE" };
    format!("{}{}", compact_scene_label(party, 30 - suffix.len()), suffix)
}

#[cfg(test)]
mod compatibility_label_tests {
    use super::*;
    #[test]
    fn battle_party_actions_export_every_visible_cursor_position() {
        for selected in 0..3 {
            let rows = battle_party_action_entries(selected);
            assert_eq!(rows.len(), 3);
            assert_eq!(rows.iter().filter(|s| s.starts_with('>')).count(), 1);
            assert!(rows[selected].starts_with('>'));
            assert_eq!(rows[2].trim_start_matches('>').trim(), "CANCEL");
        }
    }
    #[test]
    fn long_party_rows_keep_cursor_and_complete_compatibility() {
        for (able, suffix) in [(true, " ABLE"), (false, " NOT ABLE")] {
            let row = compatibility_party_label(">FERALIGATR L100 HP 999/999 PAR", able);
            assert_eq!(row.chars().count(), 30);
            assert!(row.starts_with('>'));
            assert!(row.ends_with(suffix));
        }
        assert_eq!(compatibility_party_label(" PIKACHU", false), " PIKACHU NOT ABLE");
    }
}
    ...
    if mode == FieldPackTargetMode::TmHmPokemon {
        ...
        entries.extend(
            windowed_index_range(selected_party, snapshot.party.slots.len()).map(|index| {
                ...
                compatibility_party_label(&party_slot_entry(snapshot, slot, index == selected_party), able)
            }),
        );
        return Ok(entries.into_iter().take(SCENE_MENU_VISIBLE_ROWS).collect());
    }
...
fn battle_anim_render_bundle(
        ...
        && rendered_art.battle_anim_bundle_error.is_none()
    {
        ...
        {
            Ok(bundle) => rendered_art.battle_anim_bundle_cache = Some(Arc::new(bundle)),
...
fn visible_battle_menu_surface(runtime: &BevyRuntimeShell) -> &'static str {
    if runtime.battle_faint_prompt_cursor.is_some() { "faint_prompt" }
    else if runtime.battle_shift_prompt_cursor.is_some() { "shift_prompt" }
    else if runtime.battle_pack_target_mode.is_some() { "item_target" }
    else if runtime.field_pack_action_cursor.is_some() { "item_actions" }
    else if runtime.battle_move_cursor.is_some() { "moves" }
    else if runtime.battle_switch_cursor.is_some() {
        if runtime.battle_party_action_cursor.is_some() { "party_actions" } else { "party" }
    }
    else if runtime.bag_cursor.is_some() { "items" }
    else if runtime.ball_cursor.is_some() { "balls" }
    else if runtime.key_item_cursor.is_some() { "key_items" }
    else if runtime.tmhm_cursor.is_some() { "machines" }
    else { "commands" }
}
...
fn visible_battle_command_menu_entries(
    ...
    if runtime_shell.field_pack_action_cursor.is_some() {
        return visible_field_pack_action_entries(snapshot, runtime_shell, &active_visible_field_pack_pocket(runtime_shell));
    }
    if runtime_shell.battle_move_cursor.is_some() {
        return Ok(visible_battle_move_entries(snapshot, runtime_shell, battle));
    }
    if runtime_shell.battle_switch_cursor.is_some() && runtime_shell.battle_party_action_cursor.is_some() {
        let selected = strict_readonly_cursor_index(
            &runtime_shell.battle_party_action_cursor,
            "battle:party-actions",
            3,
        ).context("battle party action cursor is invalid")?;
        return Ok(battle_party_action_entries(selected));
    }
    if runtime_shell.battle_switch_cursor.is_some() {
...
// Match the visible move information panel: no hidden damage prediction.
fn visible_battle_selected_move_info(
    snapshot: &RuntimeShellSnapshot,
    runtime: &BevyRuntimeShell,
    battle: &crate::RuntimeBattleSnapshot,
) -> serde_json::Value {
    let Some(index)=strict_readonly_cursor_index(&runtime.battle_move_cursor,"battle:moves",battle.player_moves.len()+1) else {return serde_json::Value::Null;};
    let Some(selected)=battle.player_moves.get(index) else {return serde_json::Value::Null;};
    if battle.player_disabled_move.as_deref()==Some(selected.name.as_str()) {
        return serde_json::json!({"name":selected.name,"disabled":true});
    }
    let Some(data)=snapshot.moves.iter().find(|m|m.move_id==selected.name) else {return serde_json::Value::Null;};
    serde_json::json!({"name":selected.name,"type":data.move_type,"current_pp":selected.current_pp,
        "max_pp":crate::core::models::max_move_pp(data.pp,selected.pp_ups),"disabled":false})
}
