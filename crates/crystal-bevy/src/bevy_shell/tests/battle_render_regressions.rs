#[test]
fn battle_animation_bundle_is_shared_across_redraws() {
    let mut shell = route36_battle_shell_for_render_regression();
    let snapshot = cached_runtime_snapshot(&mut shell).unwrap();
    let mut art = RenderedTilesetArt::default();
    let bundle = battle_anim_render_bundle(&mut art, &snapshot).unwrap();
    for _ in 0..60 {
        let next = battle_anim_render_bundle(&mut art, &snapshot).unwrap();
        assert!(Arc::ptr_eq(&bundle, &next), "redraw must reuse the decoded animation bundle");
    }
    let start = std::time::Instant::now();
    for _ in 0..100 {
        std::hint::black_box(bundle.as_ref().clone());
    }
    let deep_copy = start.elapsed();
    let start = std::time::Instant::now();
    for _ in 0..100 {
        std::hint::black_box(battle_anim_render_bundle(&mut art, &snapshot).unwrap());
    }
    eprintln!("100 animation bundle reads: deep={deep_copy:?}, shared={:?}", start.elapsed());
}

#[test]
fn battle_animation_preserves_shared_retained_scenes() {
    let mut shell = route36_battle_shell_for_render_regression();
    let cached = cached_runtime_snapshot(&mut shell).unwrap();
    let revision = shell.snapshot_revision;
    advance_visible_battle_transition(&mut shell);
    assert!(Arc::ptr_eq(&cached, &cached_runtime_snapshot(&mut shell).unwrap()),
        "battle intro frames must reuse the runtime snapshot");
    assert_ne!(revision, shell.snapshot_revision, "intro still needs to redraw");
    let start = std::time::Instant::now();
    for _ in 0..100 {
        std::hint::black_box(shell.shell.presentation_snapshot().unwrap());
    }
    let rebuild = start.elapsed();
    let start = std::time::Instant::now();
    for _ in 0..100 {
        std::hint::black_box(cached_runtime_snapshot(&mut shell).unwrap());
    }
    eprintln!("100 battle intro snapshots: rebuild={rebuild:?}, cached={:?}", start.elapsed());
    let scene = shell.battle_message_scene.clone().unwrap();
    let before = scene.as_ref().clone();
    shell.battle_message_scenes.push_back(Arc::clone(&scene));
    ...
    for _ in 0..8 {
        ...
        assert!(Arc::ptr_eq(shell.battle_message_scene.as_ref().unwrap(), &scene));
        assert!(Arc::ptr_eq(shell.battle_message_scenes.back().unwrap(), &scene));
    }
    assert_eq!(*scene, before, "animation must not mutate a queued scene");

    let start = std::time::Instant::now();
    for _ in 0..1000 {
        std::hint::black_box(scene.as_ref().clone());
    }
    let deep_copy = start.elapsed();
    let start = std::time::Instant::now();
    for _ in 0..1000 {
        std::hint::black_box(Arc::clone(&scene));
    }
    eprintln!("1000 retained battle scene copies: deep={deep_copy:?}, shared={:?}", start.elapsed());
}
...
fn blackout_keeps_the_last_hit_scene_until_recovery() {
    let mut shell = route36_battle_shell_for_render_regression();
    Arc::make_mut(shell.battle_message_scene.as_mut().unwrap()).battle
        .as_mut()
        .unwrap()
        .enemy_pokemon
        .hp = 1;
...
fn trainer_result_portrait_slides_in_before_defeat_text_and_retains_player() {
    ...
    shell.battle_player_send_out_pending = false;
    shell.battle_messages.clear();
    let scene = Arc::make_mut(shell.battle_message_scene.as_mut().unwrap());
...
fn battle_dialogue_uses_player_input_drains_once_and_returns_menu_control() {
    ...
    runtime_shell.battle_message_scene = Some(Arc::new(
...
fn caught_capture_retains_then_clears_sprites_without_revealing_enemy_before_commit() {
    ...
    runtime_shell.battle_message_scene = Some(Arc::new(battle_scene));
...
fn empty_terminal_battle_reward_starts_plain_map_reload_before_releasing_frame() {
    ...
    runtime_shell.battle_message_scene = Some(Arc::new(terminal_scene));
...
fn final_wild_attack_retains_move_and_faint_presentation_before_rewards() {
    ...
    mark_runtime_snapshot_dirty(&mut shell);
    shell.battle_message_scene = Some(Arc::new(shell.shell.snapshot().unwrap()));
    shell.active_music = Some("MUSIC_JOHTO_WILD_BATTLE".into());
    resolve_visible_battle_move(&mut shell, 0).unwrap();
    assert_eq!(shell.active_music.as_deref(), Some("MUSIC_JOHTO_WILD_BATTLE"));
    ...
    for _ in 0..2000 {
        ...
        if faint_pending {
            assert_eq!(shell.active_music.as_deref(), Some("MUSIC_JOHTO_WILD_BATTLE"),
                "victory music must not reveal the outcome before the final hit and faint finish");
        }
...
#[test]
fn battle_switch_menu_hides_hud_and_cancel_restores_it() {
    let mut shell = route36_battle_shell_for_render_regression();
    shell.visible_battle_transition = None;
    shell.visible_battle_sliding_intro = None;
    shell.battle_entry_messages_remaining = 0;
    ...
    shell.battle_text_reveal = None;
    sync_visible_battle_action_cursor(&mut shell);
    open_visible_battle_switch_target(&mut shell).unwrap();
    let mut app = battle_render_regression_app(shell);
    app.update();
    let world = app.world_mut();
    assert!(world.resource::<BevyRuntimeShell>().last_error.is_none());
    let mut hud = world.query_filtered::<&Transform, With<BattleHudMarker>>();
    let party_hp_y = battle_hud_tile_origin(11.0, 2.0).1;
    assert_eq!(hud.iter(world).count(), 3 + BATTLE_HUD_HP_BAR_LENGTH_TILES as usize,
        "only the party member's HP bar should remain");
    assert!(hud.iter(world).all(|transform| transform.translation.y == party_hp_y),
        "battle HUD must not overlap party rows");
    save_live_battle_canvas_for_test(world, "battle-switch-menu.png");
    press_visible_battle_b_button(&mut world.resource_mut::<BevyRuntimeShell>()).unwrap();
    app.update();
    let world = app.world_mut();
    assert!(world.resource::<BevyRuntimeShell>().last_error.is_none());
    assert!(world.query_filtered::<Entity, With<BattleHudMarker>>()
        .iter(world).count() > 0, "cancel must restore the battle HUD");
}

#[test]
fn battle_party_submenu_export_follows_open_and_cancel() {
    let mut shell = route36_battle_shell_for_render_regression();
    shell.visible_battle_transition = None;
    shell.visible_battle_sliding_intro = None;
    shell.battle_entry_messages_remaining = 0;
    ...
    sync_visible_battle_action_cursor(&mut shell);
    open_visible_battle_switch_target(&mut shell).unwrap();
    assert_eq!(visible_battle_menu_surface(&shell), "party");
    open_visible_battle_party_action_menu(&mut shell).unwrap();
    assert_eq!(visible_battle_menu_surface(&shell), "party_actions");
    let snapshot = shell.shell.snapshot().unwrap();
    let battle = snapshot.battle.as_ref().unwrap();
    assert_eq!(visible_battle_command_menu_entries(&snapshot, &shell, battle).unwrap(),
        vec![">SWITCH", " STATS", " CANCEL"]);
    press_visible_battle_b_button(&mut shell).unwrap();
    let rows = visible_battle_command_menu_entries(&snapshot, &shell, battle).unwrap();
    assert!(!rows.iter().any(|s| s.trim_start_matches('>').trim() == "SWITCH"));
    assert!(shell.battle_switch_cursor.is_some());
    assert_eq!(visible_battle_menu_surface(&shell), "party");
}

#[test]
fn battle_move_information_matches_selected_visible_panel() {
    let mut shell=route36_battle_shell_for_render_regression();
    open_visible_battle_move_target(&mut shell).unwrap();
    let snapshot=shell.shell.snapshot().unwrap();
    let mut battle=snapshot.battle.clone().unwrap();
    let selected=&battle.player_moves[0];
    let info=visible_battle_selected_move_info(&snapshot,&shell,&battle);
    assert_eq!(info["name"],selected.name);
    assert_eq!(info["current_pp"],selected.current_pp);
    assert_eq!(info["disabled"],false);
    assert!(info["type"].is_string());
    battle.player_disabled_move=Some(selected.name.clone());
    let disabled=visible_battle_selected_move_info(&snapshot,&shell,&battle);
    assert_eq!(disabled["disabled"],true);
    assert!(disabled.get("current_pp").is_none());
    shell.battle_move_cursor=None;
    assert!(visible_battle_selected_move_info(&snapshot,&shell,&battle).is_null());
}

#[test]
fn battle_potion_a_opens_use_then_party_target_with_foreground_export() {
    let mut shell=route36_battle_shell_for_render_regression();
    shell.shell.add_bag_item("POTION",1).unwrap();
    shell.visible_battle_transition=None;
    shell.visible_battle_sliding_intro=None;
    shell.battle_entry_messages_remaining=0;
    shell.battle_player_send_out_pending=false;
    shell.battle_enemy_send_out_pending=false;
    shell.battle_messages.clear();shell.battle_message_scenes.clear();shell.battle_text_reveal=None;
    let snapshot=shell.shell.snapshot().unwrap();
    let index=carried_battle_non_ball_item_ids(&snapshot).iter().position(|s|s=="POTION").unwrap();
    shell.bag_cursor=Some(MenuCursor{surface_id:"battle:bag-items".into(),option_index:index});
    for id in ["POTION","SUPER_POTION","MAX_POTION","FULL_RESTORE"] {
        assert!(item_targets_party_pokemon_fields(snapshot.items.iter().find(|i|i.item_id==id).unwrap()),"{id} must target a party member");
    }
    press_visible_battle_a_button(&mut shell).unwrap();
    assert_eq!(visible_battle_menu_surface(&shell),"item_actions");
    let snapshot=shell.shell.snapshot().unwrap();
    let rows=visible_battle_command_menu_entries(&snapshot,&shell,snapshot.battle.as_ref().unwrap()).unwrap();
    assert!(rows.iter().any(|s|s==">USE"),"{rows:?}");
    assert!(!rows.iter().any(|s|s.contains(" x1")),"Must export actions, not the obscured item list");
    press_visible_battle_a_button(&mut shell).unwrap();
    assert_eq!(visible_battle_menu_surface(&shell),"item_target");
    assert_eq!(shell.battle_pack_target_mode,Some(BattlePackTargetMode::PartyPokemon));
}

#[cfg(feature = "fullscreen-scaling")]
#[test]
fn battle_redraws_remove_retired_sprites_from_fullscreen_parents() {
    let mut shell = route36_battle_shell_for_render_regression();
    shell.visible_battle_transition = None;
    shell.visible_battle_sliding_intro = None;
    let mut app = menu_render_test_app(shell);
    ...
    for frame in 0..120 {
        // Exercise replacement of HUD, battlers and text under the same roots
        // used by the browser in classic 2D mode.
        mark_runtime_presentation_dirty(&mut app.world_mut().resource_mut::<BevyRuntimeShell>());
        app.update();
        let world = app.world_mut();
        assert!(world.resource::<BevyRuntimeShell>().last_error.is_none());
        let mut parents = world.query::<(Entity, &Children)>();
        let mut child_count = 0;
        for (parent, children) in parents.iter(world) {
            child_count += children.len();
            for child in children.iter() {
                assert_eq!(world.get::<Parent>(*child).map(Parent::get), Some(parent),
                    "frame {frame}: parent retained retired battle sprite {child:?}");
            }
        }
