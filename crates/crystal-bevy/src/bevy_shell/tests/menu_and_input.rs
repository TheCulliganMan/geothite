...
fn bills_pc_withdraw_plays_cry_and_holds_the_got_message() {
    ...
    let snapshot=runtime_shell.shell.snapshot().unwrap();
    let menu=visible_pc_observation_menu(&snapshot,&runtime_shell).unwrap().unwrap();
    assert_eq!(menu["surface"],"withdraw");
    assert_eq!(menu["selected_slot"],1);
    let party_before=snapshot.party.slots.len();
    open_visible_bill_pc_pokemon_actions(&mut runtime_shell).unwrap();
    let menu=visible_pc_observation_menu(&snapshot,&runtime_shell).unwrap().unwrap();
    assert_eq!(menu["surface"],"pokemon_actions");
    assert!(menu["entries"].as_array().unwrap().iter().any(|row|row.as_str()==Some(">WITHDRAW")));
    confirm_visible_bill_pc_pokemon_action(&mut runtime_shell).expect("withdraw selected Pokemon");
    let snapshot=runtime_shell.shell.snapshot().unwrap();
    assert_eq!(snapshot.party.slots.len(),party_before+1);
    assert_eq!(snapshot.storage.boxes[0].count,1);
    assert!(visible_pc_observation_menu(&snapshot,&runtime_shell).unwrap().is_none(),"No stale action menu during transfer animation");
...
#[test]
fn pc_observation_reports_hub_and_storage_cursor_without_mutating_them() {
    let mut shell=initialized_mail_reader_shell("FLOWER_MAIL");
    shell.field_notice=None;
    shell.pc_hub_cursor=Some(MenuCursor{surface_id:"pc:hub".into(),option_index:0});
    let snapshot=shell.shell.snapshot().unwrap();
    let menu=visible_pc_observation_menu(&snapshot,&shell).unwrap().unwrap();
    assert_eq!(menu["surface"],"hub");
    assert!(menu["entries"].as_array().unwrap().iter().any(|v|v.as_str().is_some_and(|s|s.starts_with('>'))));
    shell.pc_hub_cursor=None;
    shell.bill_pc_action_cursor=Some(MenuCursor{surface_id:"pc:bill-actions".into(),option_index:0});
    let menu=visible_pc_observation_menu(&snapshot,&shell).unwrap().unwrap();
    assert_eq!(menu["surface"],"storage_actions");
    assert!(menu["entries"].as_array().unwrap().iter().any(|v|v.as_str().is_some_and(|s|s.contains("WITHDRAW"))));
    assert_eq!(shell.bill_pc_action_cursor.as_ref().unwrap().option_index,0);
    shell.field_notice=Some("Accessing PC…".into());
    assert!(visible_pc_observation_menu(&snapshot,&shell).unwrap().is_none());
}

#[test]
fn opening_party_move_reorder_retains_a_valid_cursor_through_input() {
    let mut shell=initialized_mail_reader_shell("FLOWER_MAIL");
    // The mail fixture does not need moves; a reorder fixture must have them.
    shell.shell.session_mut().state_mut().storage.party.pokemon[0].as_mut().unwrap().moves=vec![
        crate::core::models::LearnedMove{name:"TACKLE".into(),current_pp:35,pp_ups:0},
        crate::core::models::LearnedMove{name:"LEER".into(),current_pp:30,pp_ups:0},
    ];
    shell.shell.session_mut().state_mut().sync_party_from_storage();
    mark_runtime_snapshot_dirty(&mut shell);
    shell.party_menu_open=true; shell.party_cursor=0;
    open_visible_party_move_reorder(&mut shell).unwrap();
    sync_visible_battle_action_cursor(&mut shell);
    let snapshot=shell.shell.snapshot().unwrap();
    visible_party_menu_entries(&snapshot,&shell).unwrap_or_else(|e| panic!("opened cursor {:?}: {e}",shell.party_move_cursor));
    move_visible_primary_cursor_down(&mut shell).unwrap();
    sync_visible_battle_action_cursor(&mut shell);
    let snapshot=shell.shell.snapshot().unwrap();
    visible_party_menu_entries(&snapshot,&shell).unwrap_or_else(|e| panic!("moved cursor {:?}: {e}",shell.party_move_cursor));
}

#[test]
fn field_move_selection_survives_inactive_battle_sync() {
    let mut shell=initialized_mail_reader_shell("FLOWER_MAIL");
    shell.field_pack_target_mode=Some(FieldPackTargetMode::PartyMove);
    shell.party_move_cursor=Some(MenuCursor{surface_id:party_move_cursor_surface_id(0),option_index:0});
    sync_visible_battle_action_cursor(&mut shell);
    assert!(shell.party_move_cursor.is_some(), "field PP-item cursor must survive");
    shell.field_pack_target_mode=None;
    shell.battle_pack_target_mode=Some(BattlePackTargetMode::PartyMove);
    sync_visible_battle_action_cursor(&mut shell);
    assert!(shell.party_move_cursor.is_none(), "stale battle-owned cursor must clear");
}
