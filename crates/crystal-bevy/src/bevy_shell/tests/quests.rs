...
fn tm_mart_unlocks_and_visible_transactions_preserve_money_and_inventory() {
    ...
    for (map, script, mart, headbutt, rock_smash) in [
        ...
        {
            let shell = app.world().resource::<BevyRuntimeShell>();
            let view = webmcp_observation(shell, None).unwrap();
            let menu = view["observe"]["menus"].as_array().unwrap().last().unwrap();
            assert_eq!(menu["kind"], "shop");
            assert_eq!(menu["surface"], "top");
            assert!(menu["entries"].as_array().unwrap().iter().any(|s| s == ">BUY"));
        }
        ...
        for (index, item) in inventory.iter().enumerate() {
            ...
            tm_mart_key_for_test(&mut app, KeyCode::KeyZ);
            {
                ...
                let view = webmcp_observation(shell, None).unwrap();
                let menu = view["observe"]["menus"].as_array().unwrap().last().unwrap();
                assert_eq!(menu["surface"], "quantity");
                assert_eq!(menu["item"], item.as_str());
                assert_eq!(menu["quantity"], 1);
                assert_eq!(menu["total_price"], price);
                assert_eq!(menu["entries"], serde_json::json!([]));
            }
            tm_mart_key_for_test(&mut app, KeyCode::KeyZ);
            {
                let shell = app.world().resource::<BevyRuntimeShell>();
                let view = webmcp_observation(shell, None).unwrap();
                let menu = view["observe"]["menus"].as_array().unwrap().last().unwrap();
                assert_eq!(menu["surface"], "confirm");
                assert_eq!(menu["entries"], serde_json::json!([">YES", " NO"]));
...
#[test]
fn ball_restock_purchase_exposes_quantity_and_spends_exact_money() {
    let mut shell = progression_shell_on_map_for_test("AzaleaMart");
    // This controlled post-opening fixture matches the restocking curriculum prerequisite.
    shell.shell.session_mut().state_mut().flags.set_event_flag("EVENT_GAVE_MYSTERY_EGG_TO_ELM", true).unwrap();
    shell.shell.session_mut().state_mut().money = 1000;
    let runtime = shell.runtime.clone();
    let (state, overworld) = shell.shell.session_mut().state_and_overworld_mut();
    runtime.data().transition_overworld_session(state, overworld, "AzaleaMart", TilePosition::new(3, 3),
        crate::core::systems::map_context::SpawnMemoryUpdate::Preserve, &runtime.music_ids()).unwrap();
    shell.shell.session_mut().overworld_mut().player.facing = Direction::Left;
    assert_eq!(shell.shell.current_overworld_interaction_checked().unwrap().unwrap().script,"AzaleaMartClerkScript");
    assert_eq!(quest_item_quantity(&shell, "POKE_BALL"), 0);
    if let Ok(directory) = std::env::var("POKEGEAR_PC_RENDER_DIR") {
        std::fs::create_dir_all(&directory).unwrap();
        shell.shell.save(PathBuf::from(directory).join("ball-mart-browser.crystalsave")).unwrap();
    }
    quest_talk(&mut shell, "AzaleaMartClerkScript");
    let mut app = menu_render_test_app(shell);
    quest_settle(&mut app, true, |shell| shell.shop_welcome_seen && shell.shop_top_cursor.is_some() && shell.shop_notice.is_none() && shell.field_notice.is_none());
    tm_mart_key_for_test(&mut app, KeyCode::KeyZ);
    let index = {
        let shell = app.world().resource::<BevyRuntimeShell>();
        shell.shell.snapshot().unwrap().pending_shop.unwrap().inventory.iter().position(|s|s=="POKE_BALL").unwrap()
    };
    for _ in 0..index {tm_mart_key_for_test(&mut app, KeyCode::ArrowDown);}
    tm_mart_key_for_test(&mut app, KeyCode::KeyZ);
    for _ in 0..2 {tm_mart_key_for_test(&mut app, KeyCode::ArrowUp);}
    {
        let shell = app.world().resource::<BevyRuntimeShell>();
        let view=webmcp_observation(shell,None).unwrap();
        let menu=view["observe"]["menus"].as_array().unwrap().last().unwrap();
        assert_eq!(menu["surface"],"quantity");
        assert_eq!(menu["item"],"POKE_BALL");
        assert_eq!(menu["quantity"],3);
        assert_eq!(menu["total_price"],600);
    }
    tm_mart_key_for_test(&mut app, KeyCode::KeyZ);
    tm_mart_key_for_test(&mut app, KeyCode::KeyZ);
    let shell = app.world().resource::<BevyRuntimeShell>();
    assert_eq!(quest_item_quantity(shell,"POKE_BALL"),3);
    assert_eq!(shell.shell.session().state().money,400);
    let view=webmcp_observation(shell,None).unwrap();
    assert_eq!(view["observe"]["menus"].as_array().unwrap().last().unwrap()["surface"],"notice");
}
