...
fn cherrygrove_can_lose_battle_resumes_the_opposite_asm_branch_without_whiteout() {
    ...
    assert_eq!(
        shell.snapshot().expect("live rival battle").battle.expect("active rival").battle_type,
        "BATTLETYPE_CANLOSE",
        "the real battle must retain the authored can-lose type, not just accept a forced completion flag"
    );
    let completion = shell
        .complete_scripted_trainer_battle(
            &battle_step.origin_map_name,
