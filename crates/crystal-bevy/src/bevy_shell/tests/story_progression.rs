...
fn elms_aide_verbose_potion_stops_before_the_following_dialogue() {
    ...
    for _ in 0..128 {
        tick_visible_field_text_reveal(&mut runtime_shell, true).expect("print item receipt");
    }
    press_visible_b_button(&mut runtime_shell).expect("B acknowledges the item receipt");
    assert_eq!(runtime_shell.visible_field_item_notice.as_ref().map(|n| &n.phase),
        Some(&VisibleFieldItemPhase::PocketText), "B must preserve the item prompt state machine");
    for _ in 0..8 {
        if runtime_shell.visible_field_item_notice.is_none() { break; }
        for _ in 0..128 {
            tick_visible_field_text_reveal(&mut runtime_shell, true).expect("print pocket notice");
        }
        press_visible_b_button(&mut runtime_shell).expect("B completes a pocket notice page");
    }
    assert!(runtime_shell.visible_field_item_notice.is_none());
    let snapshot = runtime_shell.shell.snapshot().expect("following aide dialogue");
    assert_eq!(snapshot.ui.text.as_ref().map(|t| t.label.as_str()), Some("AideText_AlwaysBusy"));
