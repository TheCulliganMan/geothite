...
impl OverworldSession {
    ...
    /// TryStep, so they skip destination terrain and ledge checks. Object
    /// occupancy still applies to prevent overlapping NPCs. Ice continues
    /// through TryStep and retains ordinary collision behavior.
    pub fn forced_tile_step_checked(
        ...
        if let Some(occupied) = self
            ...
            .into_iter()
            .find(|occupied| occupied.tile == to)
        {
            self.frame += 1;
            return Ok(StepOutcome::BlockedByObject {
                at: to,
                facing: direction,
                object_identifier: occupied.object_identifier,
            });
        }
        self.player.tile = to;
        self.update_follow_after_entity_move("PLAYER", from, to);
...
mod tests {
    ...
    #[test]
    fn forced_steps_respect_current_and_departing_npc_tiles() {
        for departing in [false, true] {
            let mut session = OverworldSession::with_events_and_objects(
                map_with_blocks(10, 5, vec![0; 50]),
                MapEvents::default(),
                vec![object("NPC", 6, 5, "-1")],
                tileset(),
                TilePosition::new(5, 5),
            );
            if departing {
                session
                    .set_object_runtime_tile("NPC", TilePosition::new(7, 5))
                    .unwrap();
                session
                    .object_last_runtime_tiles
                    .insert("NPC".into(), TilePosition::new(6, 5));
                session
                    .object_last_tiles_occupied_until_frame
                    .insert("NPC".into(), 8);
            }
            let outcome = session
                .forced_tile_step_checked(Direction::Right, StepOptions::default())
                .unwrap();
            assert_eq!(
                outcome,
                StepOutcome::BlockedByObject {
                    at: TilePosition::new(6, 5),
                    facing: Direction::Right,
                    object_identifier: Some("NPC".into()),
                }
            );
            assert_eq!(session.player.tile, TilePosition::new(5, 5));
            assert_eq!(session.player_last_runtime_tile, None);
            ...
            if departing {
                ...
                assert_eq!(session.player.tile, TilePosition::new(6, 5));
            }
        }
    }
