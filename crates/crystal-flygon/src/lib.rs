...
pub const INTERFACE_ID: &str = "flygon-local-prosthesis-v5-dual-mbon-operant-v3";
...
impl Brain {
    ...
    pub fn new(data: &[u8], metadata_json: &str, config_json: &str) -> Result<Brain, String> {
        ...
        let voltage = kc
            ...
            .collect();
        Ok(Self {
            operant_state: operant::OperantState::default(),
            graph_id,
            metadata,
            config,
...
impl Brain {
    fn snapshot(&self, include_spikes: bool) -> Result<String, String> {
        ...
        serde_json::to_string(&serde_json::json!({"tick":self.tick,"neural_ms":self.tick as f64*self.config.dt_ms as f64,
            ...
            "plastic_edges":self.plastic.len(),"changed_edges":changed,"dan_spikes":self.delivered_dan_spikes,
