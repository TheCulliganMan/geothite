...
impl RuntimeGameShell {
    ...
    pub fn save(&mut self, path: impl AsRef<Path>) -> Result<()> {
        // Ordinary movement updates live object structs without rewriting the
        // callback-time map-object image in GameState. Save the current roster,
        // positions and movement state or Continue can drop nearby NPCs.
        crystal_core::systems::map_context::sync_state_object_overrides(
            &mut self.session.state,
            &self.session.overworld,
        ).context("capture live NPC state before saving")?;
        let paused_before = self.session.state.game_logic_paused;
        self.set_game_logic_paused(true)?;
...
impl CrystalRuntime {
    ...
    pub fn scripted_trainer_battle_keys(&self) -> BTreeSet<RuntimeScriptedTrainerBattleKey> {
        ...
        for (map_name, module) in &self.data.maps {
            keys.extend(module.scripted_trainer_battles.iter().filter_map(|battle| {
                let request = self.data.scripted_trainer_battle_request(
                    map_name, &battle.source_script, battle.startbattle_command_index,
                ).ok()?;
                Some(RuntimeScriptedTrainerBattleKey {
                    map_name: map_name.clone(),
