...
Both control modes encode the observed local map: facing direction, nearby object
positions, and up to 13×13 terrain samples with collision permissions and map
boundaries, relative to the player. Action-memory mode uses `spatial-map-v2`:
eight separate channels for location/dialogue, facing, near north/east/south/west,
surrounding terrain, and objects. Near terrain covers two tiles from the player;
the remaining observed terrain and the player's tile form the surroundings channel.
With the default eight-cell cue, each channel addresses one real KC in a disjoint
anatomical pool. Changing one channel retains the other seven inputs, allowing
shared local geometry to reuse neural inputs across locations. This establishes
input overlap; improved navigation or learning still requires behavioral evaluation.
Larger cues distribute cells across channels; cues smaller than eight cells hash
the complete situation together so no channel is omitted. Plain laboratory cues
retain their existing encoding. The readout lists `map_features` and
`spatial_channels` for inspection. This is a structured map prosthesis, not pixel
vision or pathfinding; reward/evaluator fields are excluded. Previous cue versions
use different neural allocations, so prior learned responses do not transfer
reliably and a fresh brain is recommended for comparative runs.
...
The action-memory teacher penalizes every Select press and newly shown invalid-use
warnings (including Oak's warning), using the normal aversive pairing count. Battle
losses use twice that count and suppress simultaneous rewards, including blackout
respawn-location novelty. Warning text cannot earn NPC dialogue reward.

Twelve consecutive decisions without movement, dialogue/menu changes or battle
activity incur a mild one-pairing cost. New exploration tiles reset the inactivity
budget without giving movement rewards. Known routes allow 255 decisions without
