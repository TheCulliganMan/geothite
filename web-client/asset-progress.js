// Browser transport only; byte counts come from the response stream.
export async function fetchAsset(url, options = {}) {
  for (let attempt = 0; ; attempt++) {
    try { return await fetchAssetOnce(url, options); }
    catch (error) {
      // A changed network can interrupt large streams. Restart the transfer;
      // partial bytes are never cached or passed to the neural runtime.
      if (!(error instanceof TypeError) || attempt >= 2) throw error;
      options.report?.({ label: options.label, phase: 'connecting' });
      await new Promise(resolve => setTimeout(resolve, 500 * 2 ** attempt));
    }
  }
}
async function fetchAssetOnce(url, { label, expectedBytes, report = () => {} } = {}) {
  let loaded = 0, total = null;
  const update = phase => report({ label, loaded, total, phase });
  update('connecting');
  try {
    const response = await fetch(url);
    if (!response.ok) throw Error(`${label}: HTTP ${response.status}`);
    const size = expectedBytes ?? response.headers.get('x-asset-bytes') ??
      (!response.headers.get('content-encoding') ? response.headers.get('content-length') : null);
    total = Number(size) > 0 ? Number(size) : null;
    const buffer = total ? new Uint8Array(total) : null;
    const chunks = [];
    let last = 0;
    if (response.body) {
      const reader = response.body.getReader();
      while (true) {
        const { done, value } = await reader.read();
        if (done) break;
        if (buffer) {
          if (loaded + value.byteLength > buffer.length) throw Error(`${label}: unexpected download size`);
          buffer.set(value, loaded);
        } else chunks.push(value);
        loaded += value.byteLength;
        if (performance.now() - last > 80) { update('downloading'); last = performance.now(); }
      }
    } else {
      const bytes = new Uint8Array(await response.arrayBuffer());
      if (buffer) buffer.set(bytes); else chunks.push(bytes);
      loaded = bytes.byteLength;
    }
    if (total !== null && loaded !== total) throw Error(`${label}: incomplete download (${loaded} / ${total} bytes)`);
    const bytes = buffer || new Uint8Array(loaded);
    let offset = 0;
    for (const chunk of chunks) { bytes.set(chunk, offset); offset += chunk.byteLength; }
    update('downloaded');
    return bytes;
  } catch (error) { report({ label, loaded, total, phase: 'error', error: String(error.message || error) }); throw error; }
}
// Dataset identity, not a reusable file URL, keys the persistent large assets.
const CONNECTOME_CACHE = 'flygon-connectome-v1';
function cacheKey(url, graphId) {
  const key = new URL(url, globalThis.location.href);
  key.searchParams.set('graph', graphId);
  return key.href;
}
export async function fetchConnectomeAsset(url, options) {
  const { graphId, expectedBytes, label, report = () => {} } = options;
  try {
    const cache = await caches.open(CONNECTOME_CACHE);
    const response = await cache.match(cacheKey(url, graphId));
    if (response) {
      const bytes = new Uint8Array(await response.arrayBuffer());
      if (bytes.byteLength === expectedBytes) {
        report({ label, loaded: bytes.length, total: bytes.length, phase: 'cached' });
        return { bytes, cached: true };
      }
      await cache.delete(cacheKey(url, graphId));
    }
  } catch { /* Cache storage is optional in private/restricted browsers. */ }
  return { bytes: await fetchAsset(url, options), cached: false };
}
export async function storeConnectomeAsset(url, graphId, bytes) {
  try {
    const cache = await caches.open(CONNECTOME_CACHE);
    await cache.put(cacheKey(url, graphId), new Response(bytes, {
      headers: { 'Content-Type': 'application/octet-stream', 'Content-Length': String(bytes.length) },
    }));
    return true;
  } catch { return false; }
}
export async function evictConnectome(graphId) {
  try {
    const cache = await caches.open(CONNECTOME_CACHE);
    for (const key of await cache.keys()) {
      if (new URL(key.url).searchParams.get('graph') === graphId) await cache.delete(key);
    }
  } catch { /* The network path still works without storage. */ }
}
export async function pruneConnectomeCache(graphId) {
  try {
    const cache = await caches.open(CONNECTOME_CACHE);
    for (const key of await cache.keys()) {
      if (new URL(key.url).searchParams.get('graph') !== graphId) await cache.delete(key);
    }
  } catch { /* Best-effort cleanup after the current dataset has been stored. */ }
}
export async function loadWasm(init, url, label, report) {
  const bytes = await fetchAsset(url, { label, report });
  report({ label, loaded: bytes.byteLength, total: bytes.byteLength, phase: 'initializing' });
  try {
    const runtime = await init({ module_or_path: bytes });
    report({ label, loaded: bytes.byteLength, total: bytes.byteLength, phase: 'ready' });
    return runtime;
  } catch (error) {
    report({ label, phase: 'error', error: String(error.message || error) });
    throw error;
  }
}
export function mountAssetProgress(container, labels) {
  const rows = new Map();
  const format = bytes => `${(bytes / 1048576).toFixed(1)} MiB`;
  const report = ({ label, loaded = 0, total = null, phase = 'waiting', error }) => {
    let row = rows.get(label);
    if (!row) {
      const element = document.createElement('div'); element.className = 'asset-progress';
      const name = document.createElement('strong'); name.textContent = label;
      const value = document.createElement('span');
      const bar = document.createElement('progress'); bar.max = 1; bar.setAttribute('aria-label', label);
      element.append(name, value, bar); container.append(element);
      row = { element, value, bar }; rows.set(label, row);
    }
    row.element.dataset.phase = phase;
    const sizes = total ? `${format(loaded)} / ${format(total)}` : format(loaded);
    row.value.textContent = phase === 'downloading' ? `${sizes}${total ? ` · ${Math.floor(loaded / total * 100)}%` : ''}` :
      phase === 'downloaded' ? `${sizes} · Downloaded` :
      phase === 'ready' ? `${format(loaded)} · Ready` :
      phase === 'cached' ? `${format(loaded)} · From device cache` :
      phase === 'initializing' ? 'Initializing…' : phase === 'error' ? error : phase === 'connecting' ? 'Connecting…' : 'Waiting';
    if ((phase === 'downloading' && total) || ['downloaded', 'cached', 'ready'].includes(phase)) row.bar.value = total ? loaded / total : 1;
    else row.bar.removeAttribute('value');
  };
  labels.forEach(label => report({ label }));
  return report;
}
