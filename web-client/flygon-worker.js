...
import { fetchConnectomeAsset, storeConnectomeAsset, evictConnectome, pruneConnectomeCache, loadWasm } from "./asset-progress.js";
...
self.onmessage = async ({ data }) => {
  ...
  try {
    ...
    if (kind === "load") {
      ...
      let telemetry, cacheStatus;
      for (let attempt = 0; attempt < 2; attempt++) {
        const [graph, metadata] = await Promise.all([
          fetchConnectomeAsset(base + "graph.bin", { graphId: dataset.graph_id, label: "Connectome", expectedBytes: dataset.graph_bytes, report }),
          fetchConnectomeAsset(base + "metadata.json", { graphId: dataset.graph_id, label: "Neuron metadata", expectedBytes: dataset.metadata_bytes, report }),
        ]);
        try {
          report({ label: "Connectome", phase: "initializing" });
          brain = new Brain(graph.bytes, new TextDecoder().decode(metadata.bytes), JSON.stringify(data.config));
          telemetry = JSON.parse(brain.telemetry());
          if (telemetry.graph_id !== dataset.graph_id || telemetry.neurons !== dataset.neurons || telemetry.edges !== dataset.edges)
            throw Error("Loaded graph does not match the pinned full MaleCNS dataset");
        } catch (error) {
          brain?.free();
          brain = null;
          if (attempt === 0 && (graph.cached || metadata.cached)) {
            await evictConnectome(dataset.graph_id);
            continue;
          }
          throw error;
        }
        // Persist only after Rust verifies the combined graph/metadata digest.
        const stored = await Promise.all([
          graph.cached || storeConnectomeAsset(base + "graph.bin", dataset.graph_id, graph.bytes),
          metadata.cached || storeConnectomeAsset(base + "metadata.json", dataset.graph_id, metadata.bytes),
        ]);
        cacheStatus = graph.cached && metadata.cached ? "device cache" : stored.every(Boolean) ? "saved to device cache" : "device cache unavailable";
        if (stored.every(Boolean)) await pruneConnectomeCache(dataset.graph_id);
        brain.prepare_circuit();
        telemetry = JSON.parse(brain.telemetry());
        report({ label: "Connectome", loaded: graph.bytes.byteLength, total: graph.bytes.byteLength, phase: "ready" });
        break;
      }
      result = {
        ...telemetry,
      ...
      const previous = data.freezeLearning ? JSON.parse(brain.configuration()) : null;
      let r;
      try {
        if (previous) brain.configure(JSON.stringify({ ...previous, learning: false }));
    ...
    else if (kind === "restore" || kind === "upgrade_readout") {
      const migration = kind === "upgrade_readout"
        ? JSON.parse(brain.upgrade_readout_checkpoint(data.checkpoint))
        : (brain.restore(data.checkpoint), null);
