...
function profileSelected(observation) {
  const menu = observation?.observe?.menus?.find((menu) => menu.kind === "start");
  return menu?.entries?.some(
    (entry) => typeof entry === "string" && /^>\s*PROFILE\s*$/i.test(entry),
  );
}
const headlessTraining = new URLSearchParams(location.search).get("headless") === "1";
if (headlessTraining) $("brain-panel").hidden = true;
const viewer = headlessTraining ? null : createViewer($("brain"), {
...
function showMetrics(t) {
  showCircuits(t);
  $("metrics").textContent =
    `${t.neurons.toLocaleString()} neurons · ${t.edges.toLocaleString()} connections · ${(t.operant_trial_ms ?? t.neural_ms).toFixed(0)} ${t.operant_trial_ms ? "summed trial ms" : "neural ms"} · ${config.operant ? `${t.policy_updates ?? 0} policy updates` : `${t.changed_edges}/${t.plastic_edges} plastic edges changed`}`;
}
function showCircuits(t) {
  $("circuits").textContent =
    ...
    (t.circuit_activity || [])
      ...
      .join(" · ") +
    (config.operant ? "" : ` · Scheduled current: PAM ${((t.reward_ticks || 0) * (config?.dt_ms || 0)).toFixed(0)} ms / PPL ${((t.pulse_ticks || 0) * (config?.dt_ms || 0)).toFixed(0)} ms remaining`);
}

let controlEpoch = 0;
let scheduledStep = null;
// Read-only settling: let a released joypad action finish its field animation
// before assigning its outcome or choosing another action. No buttons are added.
async function settleFieldAction(game, observation) {
  const deadline = performance.now() + 2000;
  while (observation?.status?.screen === "overworld" &&
         observation?.flow_state?.animating && performance.now() < deadline) {
...
async function step() {
  ...
  try {
    ...
    if (!laboratory && game) {
      try {
        observation = await settleFieldAction(game, await game.execute({ kind: "observe" }));
    ...
    $("speed").textContent = config.operant
      ? `${r.wall_ms.toFixed(0)} ms sensory circuit · ${(r.wasm_memory_bytes / 1048576).toFixed(0)} MiB neural WASM memory`
    ...
    $("readouts").textContent = JSON.stringify(
      {
        ...
        spike_populations: config.operant ? ["descending_neuron"] : undefined,
        sensory: r.sensory,
        observation: {
          stale,
    ...
    if ($("play").checked && game) {
      let outcome = null;
      // A typewriter can finish and expose YES/NO while neural compute runs.
      // Never train or submit a dialogue decision against that newer choice.
      if (!stale && r.action.button && hasInputSurface(observation)) {
        try {
          const current = await game.execute({ kind: "observe" });
          if (epoch !== controlEpoch) return;
          lastObservation = current;
          if (inputContext(current) !== inputContext(observation)) {
            transport.changedInputContexts = (transport.changedInputContexts || 0) + 1;
            // The next Rust decision discards pending, unsubmitted credit.
            return;
          }
        } catch (e) {
          sensorFailure(e);
      ...
      try {
        ...
        const profileBlocked = r.action.button === "a" && profileSelected(observation);
        outcome = r.action.button
          ? await game.execute({
              kind: "press",
              button: profileBlocked ? "b" : r.action.button,
          ...
          : stale
            ...
            : observation;
        if (outcome) {
          outcome = await settleFieldAction(game, outcome);
          if (profileBlocked) {
            outcome.flygon_guard = "profile_blocked";
            log("Blocked neural input from opening PROFILE; closed the menu instead.");
          }
      ...
      if (outcome && !stale) {
        ...
        $("story-state").textContent =
          `${reward.breadcrumb?.target || "Story progress"} · ${reward.story?.milestones?.length || 0} milestones${reward.training?.population ? ` · ${reward.training.population === "PPL101" ? "Shock" : "Reward"} ${reward.training.pulse_ms} ms` : ""}${reward.events.length ? " · " + reward.events.slice(-3).join(" · ") : ""}`;
        if (reward.training)
          showMetrics({
        ...
        if (reward.events.length)
          log(
            `Outcome${reward.enabled ? (config.operant ? " → training feedback" : " → DAN pulse") : " (learning disabled)"}: ${reward.events.join(", ")}`,
          );
      }
    }
    await render().catch(e => log(`Brain view skipped: ${e.message || e}`));
    $("speed").textContent = config.operant
      ? `${r.wall_ms.toFixed(0)} ms sensory circuit · ${(performance.now() - loopStart).toFixed(0)} ms full loop · ${(r.wasm_memory_bytes / 1048576).toFixed(0)} MiB neural WASM memory`
    ...
    busy = false;
    if (running) scheduledStep = setTimeout(() => {
      scheduledStep = null;
      if (running) step();
    }, 0);
  }
}
let loadingBrain = false;
async function loadBrain() {
  ...
  try {
    ...
    syncTuning();
    log(
      `Loaded ${r.neurons} neurons and ${r.edges} edges in ${(r.load_ms / 1000).toFixed(1)} s · ${r.cache_status} · ${(r.wasm_memory_bytes / 1048576).toFixed(0)} MiB neural WASM memory (game/browser memory additional)`,
    );
    await render();
  } catch (e) {
    loaded = false;
    $("load").disabled = false;
    $("load").hidden = false;
    $("load").textContent = "Retry loading";
    error(e);
  } finally {
    loadingBrain = false;
  }
}
$("load").onclick = loadBrain;
$("run").onclick = (event) => {
  if (!running) $("game").contentWindow?.__flygonActivateAudio?.(event);
  running = !running;
  if (!running && scheduledStep !== null) { clearTimeout(scheduledStep); scheduledStep = null; }
...
async function reloadView() {
  ...
  for (const key of ["background", "inactive", "spike", "dopamine", "penalty"])
    if (
      next[key] !== undefined &&
